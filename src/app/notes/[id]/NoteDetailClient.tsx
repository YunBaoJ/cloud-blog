...
function CodeBlock({ code, lang }: { code: string; lang: string }) {
  ...
  const isLongCode = linesCount > 10;
  ...
  return (
    <>
      <div className={`relative my-6 rounded-2xl bg-[#141C16] border border-white/10 overflow-hidden shadow-xl transition-all duration-300 ${
        isFullscreen ? "hidden" : ""
      }`}>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 bg-[#0D140E] border-b border-white/10 text-xs font-mono">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-1.5 rounded-xl bg-white/8 hover:bg-white/15 text-[#9EB3A4] hover:text-white transition-all active:scale-95 border border-white/10"
              title="全屏放大模式"
            >
        ...
        {/* Code Content Container with Height Clamping */}
        <div
          className={`p-4 sm:p-5 overflow-x-auto font-mono text-xs sm:text-sm text-[#E2EBE4] leading-relaxed bg-[#141C16] transition-all duration-300 ${
            isLongCode && !isExpanded ? "max-h-[300px] overflow-hidden relative" : ""
          }`}
        >
          ...
          {isLongCode && !isExpanded && (
            <div className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-[#141C16] via-[#141C16]/80 to-transparent pointer-events-none" />
          )}
        </div>
        ...
        {isLongCode && (
          <div className="px-4 py-2.5 bg-[#0D140E] border-t border-white/10 flex items-center justify-center">
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#7CD090] hover:text-white transition-colors"
            >
              {isExpanded ? (
                <>
                  <span>收起部分代码</span>
      ...
      {isFullscreen && (
        <div className="fixed inset-0 z-[9999] bg-black/80 backdrop-blur-md p-4 sm:p-8 flex flex-col items-center justify-center animate-in fade-in duration-200">
          <div className="w-full max-w-5xl max-h-[90vh] bg-[#141C16] border border-white/15 rounded-3xl overflow-hidden shadow-2xl flex flex-col">
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-4 bg-[#0D140E] border-b border-white/10">
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={handleCopy}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/10 hover:bg-white/20 text-white text-xs font-medium transition-all"
                >
                  {copied ? (
                    <>
