...
export default function PlaygroundClient() {
  ...
  return (
    <div ref={pageRef} className="min-h-screen flex flex-col justify-between select-none">
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 sm:pt-32 pb-20 space-y-12">
        ...
        <div className="playground-page-anim space-y-3">
          ...
          <p className="text-sm sm:text-base text-[#5A5551] dark:text-[#9EB3A4] max-w-2xl leading-relaxed">
            独立复古掌机与东方博弈实验工坊。按住下方实体卡片<strong>直接拖拽至掌机外围右侧卡槽</strong>，插卡即玩、投币开机。
          </p>
        </div>

        {/* ================================================================= */}
        {/* 🎮 主工作区：左侧经典掌机本体 + 外围右侧实体卡槽拓展坞 (Console + Outer Bay) */}
        {/* ================================================================= */}
        <div className="playground-page-anim grid grid-cols-1 lg:grid-cols-[1fr_260px] xl:grid-cols-[1fr_280px] items-stretch gap-6 pt-7 sm:pt-8">
          
          {/* ============================================================= */}
          {/* 1. 掌机本体 (Classic Ergonomic Handheld Console) */}
          {/* ============================================================= */}
          <div className="relative w-full">
            ...
            {/* 掌机机身外壳 (Ergonomic Symmetrical Dual-Grip Console Body) */}
            <div className="relative z-10 rounded-[2.8rem] sm:rounded-[3.8rem] bg-[#EAE6DC] dark:bg-[#16231A] p-3 sm:p-5 sm:px-6 border-4 border-[#26352A]/20 dark:border-white/18 shadow-[0_24px_70px_rgba(38,53,42,0.2)] dark:shadow-[0_24px_70px_rgba(0,0,0,0.5)] flex flex-col md:flex-row items-center gap-4 sm:gap-6 justify-between">
              
              {/* 左侧握把区：极简模拟摇杆 + 呼吸灯 (Left Grip) */}
              <div className="hidden md:flex flex-col items-center justify-between h-[350px] w-20 shrink-0 py-3 select-none">
                {/* Power LED */}
              ...
              {/* 中央屏幕：街机待机投币 VS 游戏机运行主界面 (Display Screen) */}
              <div
                onDragOver={(e) => {
                  e.preventDefault();
                ...
                onDrop={(e) => {
                  ...
                  const gameId = (e.dataTransfer.getData("text/plain") || e.dataTransfer.getData("application/x-game-id")) as GameId;
                  if (gameId && GAMES.some((g) => g.id === gameId)) {
                    insertCartridge(gameId);
                  }
                }}
                className={`relative flex-1 w-full min-h-[360px] sm:min-h-[400px] rounded-[1.8rem] sm:rounded-[2.4rem] bg-[#0A0D0B] p-4 sm:p-6 shadow-[inset_0_4px_32px_rgba(0,0,0,0.98)] border overflow-hidden flex flex-col justify-between select-none transition-all ${
                  isDragOverBay
                    ? "border-[#7CD090] ring-4 ring-[#7CD090]/40"
                    : "border-white/10"
                }`}
              >
                    ...
                    <div className="space-y-4 py-8 max-w-sm mx-auto">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xs font-mono text-[#7A736A] dark:text-[#9EB3A4]">
                          未检测到游戏卡 · 请按住下方卡片<strong>拖入右侧外围卡槽</strong>，或点击卡片快速装载
                        </p>
                      </div>
                      <button
                        type="button"
              ...
              {/* 右侧握把区：实体 HELP 帮助键 + PLAY 确认键 (Right Grip) */}
              <div className="hidden md:flex flex-col items-center justify-between h-[350px] w-20 shrink-0 py-3 select-none">
                {/* Online Badge */}
          ...
          {/* ============================================================= */}
          {/* 2. 掌机外围右侧：独立实体卡槽拓展坞 (Outer Right Cartridge Bay) */}
          {/* ============================================================= */}
          <div
            onDragOver={(e) => {
              e.preventDefault();
            ...
            className={`relative rounded-[2.2rem] p-4 sm:p-5 flex flex-col justify-between gap-4 transition-all duration-300 border-3 isolate ${
              isDragOverBay
                ? "bg-[#36513B]/90 border-[#7CD090] ring-4 ring-[#7CD090]/50 shadow-[0_0_35px_rgba(124,208,144,0.7)] scale-[1.02]"
                : isDraggingId
                ? "bg-[#1E2E23] border-[#7CD090] animate-pulse ring-2 ring-[#7CD090]/30 shadow-lg"
                : insertedCartridge
                ? "bg-[#EAE6DC] dark:bg-[#16231A] border-[#26352A]/20 dark:border-white/18 shadow-xl"
                : "bg-[#FAF7F2] dark:bg-[#131D16] border-dashed border-[#36513B]/30 dark:border-white/20 shadow-md"
            }`}
          >
            {/* 顶部拓展坞状态标头 + 弹卡键 */}
            <div className="flex items-center justify-between border-b border-black/10 dark:border-white/10 pb-3">
              <div className="flex items-center gap-2">
                <span className={`size-2.5 rounded-full ${insertedCartridge ? "bg-[#7CD090] shadow-[0_0_8px_#7CD090]" : "bg-amber-400 animate-ping"}`} />
                <span className="font-mono text-xs font-black tracking-wider text-[#26352A] dark:text-[#F0F5F1]">
                  CARTRIDGE DOCK
                </span>
              </div>
              ...
              {insertedCartridge ? (
                <button
                  type="button"
                  onClick={ejectCartridge}
                  className="px-2.5 py-1 rounded-xl bg-red-600 hover:bg-red-500 text-white font-mono text-[10px] font-bold transition-all active:scale-90 shadow-xs cursor-pointer flex items-center gap-1"
                  title="弹出卡带 (E)"
                >
                  <span>⏏ 弹卡</span>
                </button>
            ...
            {/* 中部核心插槽 (Physical Insertion Socket) */}
            <div className="flex-1 flex flex-col justify-center items-center py-2">
              {insertedCartridge ? (
                /* 已插卡状态：真实 3D 卡带插在拓展座中 */
                <div className="w-full space-y-3 animate-in zoom-in-95 duration-200">
                  <div className={`relative rounded-2xl overflow-hidden bg-gradient-to-br ${activeGameInfo.shellColor} p-4 border-2 border-white/25 shadow-xl flex flex-col justify-between h-44`}>
                    {/* 卡身顶部防滑纹理 */}
                    ...
                    <div className="text-center space-y-1">
                      <p className="font-serif font-black text-2xl text-white tracking-widest drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
                    ...
                    {/* 底部全息激光微条与锁定状态 */}
                    <div className="space-y-1">
                      <div className="h-1.5 w-full bg-gradient-to-r from-amber-400 via-emerald-400 to-cyan-400 rounded-full opacity-80" />
                      <div className="flex items-center justify-between text-[9px] font-mono text-white/80 pt-0.5">
                        <span>{activeGameInfo.capacity}</span>
                        <span className="text-[#7CD090] font-black">● LOCKED</span>
                      </div>
                    </div>
                  </div>

                  {/* 插座深槽金属咬合线 */}
                  <div className="w-full py-1.5 px-3 rounded-lg bg-black/20 dark:bg-black/40 border border-black/10 flex items-center justify-between text-[9px] font-mono text-[#7A736A] dark:text-[#9EB3A4]">
                    <span>STATUS: INSERTED</span>
                    <span className="text-[#7CD090]">64-BIT BUS</span>
                  </div>
                </div>
              ) : (
                /* 空卡槽状态：深凹插槽与拖拽提示目标区 */
                <div className="w-full py-6 px-3 rounded-2xl bg-black/5 dark:bg-black/30 border-2 border-dashed border-[#36513B]/30 dark:border-white/20 text-center space-y-3">
                  {/* 金属触点引脚 */}
                  <div className="flex justify-center gap-1.5 opacity-40">
                    {[...Array(8)].map((_, i) => (
                      <span key={i} className="w-1.5 h-4 bg-amber-400 rounded-xs" />
                    ))}
                  </div>

                  <div className="space-y-1">
                    <p className="font-mono text-xs font-bold text-[#36513B] dark:text-[#7CD090]">
                      {isDragOverBay ? "✦ 松开即可完成插卡！" : "🖱️ 拖拽下方卡片至此处"}
                    </p>
                    <p className="text-[10px] text-[#7A736A] dark:text-[#9EB3A4]">
                      将卡片拖入此拓展坞即可插卡开机
                    </p>
                  </div>
                  ...
                  <button
                    type="button"
                    onClick={() => insertCartridge(GAMES[0].id)}
                    className="px-3.5 py-1.5 rounded-full bg-[#36513B] text-white hover:bg-[#46654C] transition-all text-[10px] font-mono font-bold inline-flex items-center gap-1 shadow-xs cursor-pointer"
                  >
                    <span>快速装载卡带 ➔</span>
                  </button>
                </div>
            ...
            {/* 底部拓展坞硬件标识 */}
            <div className="border-t border-black/10 dark:border-white/10 pt-2 flex items-center justify-between text-[9px] font-mono text-[#7A736A] dark:text-[#9EB3A4]">
              <span>EXPANSION DOCK v2</span>
              <span>RIGHT BAY</span>
            </div>
          </div>
        ...
        <div className="playground-page-anim space-y-4 pt-4">
          <div className="flex flex-wrap items-center justify-between text-xs font-mono text-[#7A736A] dark:text-[#9EB3A4] px-1 gap-2">
            ...
            <span className="text-[11px] bg-black/5 dark:bg-white/5 px-2.5 py-1 rounded-full border border-black/5 dark:border-white/5">
              🖱️ <strong>按住任意卡片拖拽</strong>至右侧外置卡槽 · 亦支持点击快速装载
            </span>
          </div>
              ...
              return (
                <div
                  ...
                  className={`group relative rounded-2xl p-4 transition-all duration-300 flex flex-col justify-between gap-3.5 cursor-grab active:cursor-grabbing isolate ${
                    isDragging
                      ? "opacity-30 scale-95 border-2 border-dashed border-[#7CD090]"
                      : isInserted
