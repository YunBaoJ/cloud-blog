...
export default function ArticleTOC({ items }: { items: TOCItem[] }) {
  ...
  return (
    <>
      {/* Desktop Fixed Outer Left Square Panel — Aligned with article card top at top-28 */}
      <aside ref={sidebarRef} className="fixed left-3 sm:left-4 md:left-6 lg:left-6 xl:left-8 2xl:left-14 top-28 z-40 w-52 hidden xl:block">
        <nav className="space-y-3 bg-white/95 dark:bg-[#1C1A17]/95 backdrop-blur-xl p-4 rounded-md border border-[#2D2B2C]/15 dark:border-white/15 shadow-[0_8px_32px_rgba(0,0,0,0.08)]">
          <div className="flex items-center justify-between border-b border-[#2D2B2C]/12 dark:border-white/15 pb-2.5">
            ...
            <span className="text-[10px] font-mono text-[#7A736A] dark:text-[#9EB3A4] px-1.5 py-0.5 rounded-sm bg-[#FAF7F2] dark:bg-[#24221F] border border-[#2D2B2C]/10 dark:border-white/10">
              {items.length} 章节
            </span>
          </div>

          <ul className="space-y-1 text-xs font-medium max-h-[65vh] overflow-y-auto pr-1">
            {items.map((item) => {
              ...
              return (
                <li key={item.id} style={{ paddingLeft: item.level === 3 ? "0.75rem" : "0rem" }}>
                  <a
                    ...
                    className={`flex items-center gap-1.5 py-1.5 px-2 rounded-sm transition-all duration-200 ${
                      isActive
